...
NODE=namada
METHOD=genesis
NAMADA_VERSION="namada-v0.22.0-Linux-x86_64"
...
if [ ! -f "$HOME/$NAMADA_VERSION.tar.gz" ]; then
    wget -P $HOME https://github.com/anoma/namada/releases/download/v0.22.0/$NAMADA_VERSION.tar.gz
    check_success
    sleep 1
else
...
if [ ! -d "$HOME/namada-testnets" ]; then
    ...
    echo_and_log "Репозиторий уже клонирован, удаляем и заново клонируем." "$YELLOW"
    rm -rf $HOME/namada-testnets
    cd $HOME
    git clone https://github.com/$USERNAME/namada-testnets $HOME/namada-testnets
    check_success
fi
